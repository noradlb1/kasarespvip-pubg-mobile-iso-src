...
namespace Offsets {
kaddr UWorld = 0x10A4D34E0;
kaddr GNames = 0x10A16C108;
kaddr LineOfSightTo_Func = 0x105F9138C;
}
...
string getSightWithFName(string FName) {
    if (isContain(FName, "BP_MZJ_3X_Pickup_C")) {
        return "3倍";//PickUpListWrapperActor
    }
...
- (void)readData
{
    ...
    auto GWorld = GetPtr(GetRealOffset(Offsets::UWorld));
...
#pragma mark
            ...
            auto rootComponent = GetPtr(props.base + 0x1d8);
            if (IsValidAddress(rootComponent)) {
                props.worldPos = Read<Vector3>(rootComponent + 0x1C0);
...
self.localPlayer.base = GetPtr(localPlayerController + 0x3f0);
...
if (IsValidAddress(self.localPlayer.base)) {
    ...
    if (isPlayer(FName)) {
        if ([self readPlayerInfo:self.localPlayer]) {
            self.isFire = Read<bool>(self.localPlayer.base + 0x1488);
            auto weaponManagerComponent = GetPtr(self.localPlayer.base + 0x2340);
            auto cachedCurUseWeapon = GetPtr(weaponManagerComponent + 0x2f0);
            auto shootWeaponComponent = GetPtr(cachedCurUseWeapon + 0xcf8);
            ownerShootWeapon = GetPtr(shootWeaponComponent + 0x1c0);
        }
        } else {
            ownerShootWeapon = 0;
        }
    ...
    auto playerCameraManager = GetPtr(localPlayerController + 0x478);
    POV = Read<MinimalViewInfo>(playerCameraManager + 0xfe0 + 0x10);
    controlRotation = localPlayerController + 0x418;
...
{
    auto rootComponent = GetPtr(player.base + 0x1d8);
    if (IsValidAddress(rootComponent)) {
        player.worldPos = Read<Vector3>(rootComponent + 0x1C0);
       ...
       player.hp = Read<float>(player.base + 0xba8);

    player.maxHp = Read<float>(player.base + 0xbb0);
    
    player.signalHP = Read<float>(player.base + 0xbb4);
    player.signalHPMax = Read<float>(player.base + 0xbc0);
    player.isFallDown = player.hp == 0;
    

    player.isDead = Read<bool>(player.base + 0xc10);
    ...
    auto nameAddr = GetPtr(player.base + 0x850);
    ...
    player.isAI = Read<bool>(player.base + 0x8dc);

    player.teamNo = Read<int>(player.base + 0x8c0);
...
{
    auto mesh = GetPtr(base + 0x470);
    if (IsValidAddress(mesh)) {
        FTransform meshTrans = Read<FTransform>(mesh + 0x1b0);
        FMatrix c2wMatrix = TransformToMatrix(meshTrans);
        auto boneArray = GetPtr(mesh + 0x6D8);
...
{
    auto mesh = GetPtr(player.base + 0x470);
    if (IsValidAddress(mesh)) {
        FTransform meshTrans = Read<FTransform>(mesh + 0x1b0);
        FMatrix c2wMatrix = TransformToMatrix(meshTrans);
        auto boneArray = GetPtr(mesh + 0x6D8);
...
- (void)modifyWeaponData
{
    if (IsValidAddress(ownerShootWeapon)) {
        kaddr shootWeaponEntityComp = GetPtr(ownerShootWeapon + 0xec0);
...
- (void)aimbot
{
    ...
    if (self.isFire) {
        if (IsValidAddress(self.lockActor)) {
            float targetHp = Read<float>(self.lockActor + 0xba8);
            float localPlayerHp = Read<float>(self.localPlayer.base + 0xba8);
